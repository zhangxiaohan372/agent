class PromptManager:
    ...
    def get_system_prompt(self):
        ...
        - 当用户初次提出登记需求时，提炼出名称、种类、所在区域、健康状况等信息，向用户发起确认提示，例如：“即将向系统添加猫咪【小橘】，地点【图书馆草坪】，是否确认提交？”。
        - 在用户明确回复“确认/提交/是的”之前，严禁假装已录入。
    ...
    def get_router_prompt(self, tool_list):
        ...
        return (
         ...
         - 针对数据持久化写入工具（如 register_pet）：
           1. 当用户初次提出登记动物（如“帮我登记一只在图书馆的橘猫小橘”），或补充信息但【尚未进行最终确认】时：
              - 严禁调用 register_pet！
              - 必须返回 CHAT，由助手在对话中反问用户确认（如“即将向系统添加猫咪【小橘】，地点【图书馆草坪】，是否确认提交？”）。
           2. 只有当历史对话中助手刚刚发起了确认提示（如询问是否确认提交），且用户当前轮明确回复“确认”、“提交”、“是的”、“同意”、“确定”等肯定指令时：
              - 此时必须返回 TOOL，name 为 "register_pet"，并将之前提炼好的完整参数（pet_type, name, area, breed, health_status 等）填入 args。
         ...
         {{
            "routes":[{{"type":"CHAT","query":"向用户核对小橘猫咪信息并反问确认是否提交","priority":1}}]
         }}

         历史对话中助手已询问：“即将向系统添加猫咪【小橘】，地点【图书馆草坪】，是否确认提交？”
         用户：确认提交
         （用户已确认，触发真实写入）
         {{
            "routes":[{{"type":"TOOL","name":"register_pet","args":{{"pet_type":"cat","name":"小橘","area":"图书馆草坪","breed":"中华田园橘猫","health_status":"健康"}},"priority":1}}]
         }}
